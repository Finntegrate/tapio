...
**Known limitation — a single check failing open.** `classify()` does not distinguish "this check ran and found no match" from "this check's model call failed and was treated as no match" — both fold into the same `None`. If, say, only the crisis check fails (a transient error or a schema-mode quirk on that one call) while the other two checks run normally and the model can still generate ordinary answers, a crisis message could reach RAG and get an ordinary, unqualified answer instead of being intercepted. This was raised in PR #141 review with a concrete suggestion: treat a failed crisis check as "crisis, conservatively" rather than "no match."

That specific fix was deliberately not made. A crisis check can fail open for reasons that have nothing to do with the message — a slow model, a transient parse error, an Ollama hiccup — and those are not rare relative to how often a message is an actual crisis. Treating every such failure as a crisis match would mean a noticeable fraction of ordinary conversations occasionally get intercepted with emergency-line contacts for no reason a user could see, which has its own cost (confusion, eroded trust in the guardrail, a worse experience for the vast majority of unaffected users) — that tradeoff was not judged worth trading away for the additional case caught. The scenario is also self-limiting: `LLMGuardrailClassifier` and the RAG pipeline's `LLMService` both depend on the same Ollama connection, so a failure severe enough to affect one check but not ordinary generation is a narrower band of possible failure than it might sound.

Whether a "classifier degraded" signal with a more measured response (e.g. a disclaimer, or a second attempt) is worth the added complexity is still an open question, not resolved here. It hasn't been closed off, either: `_run_check` already returns `None` for exactly this case, so a future change distinguishing "no match" from "failed" would build on the existing structure rather than reworking it.
