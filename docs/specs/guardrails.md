...
- An **infra failure** — `httpx.RequestError`, a provider-specific connection error (`ollama.ResponseError`, `openai.APIConnectionError`, `anthropic.APIConnectionError`), or the check's own `asyncio.timeout` firing — is message-independent. It also means the RAG pipeline's own chat model call is likely to hit the same failure right after (both depend on the same configured provider connection, whichever `TAPIO_LLM_PROVIDER` selects — #9), ending the turn at the existing generic-error path regardless of what the classifier does. These fail open immediately, no retry: the same degradation the RAG pipeline already has for an unreachable model.
