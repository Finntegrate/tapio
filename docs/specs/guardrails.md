...
This code deliberately does **not** withhold (fail closed on) draft resources — a crisis/legal-sensitive match always surfaces its matching entries regardless of `status`. The entries themselves were populated from primary-source lookups (each organization's own site, per the governance doc), not placeholders; withholding a correctly-sourced emergency number from someone in crisis, in order to satisfy a governance process that hasn't finished yet, would trade a real, immediate harm for a procedural one. The loud runtime warning plus this documented limitation are the intended control for the draft period — not a code-level block — and the actual production gate is the human sign-off itself. A future change to enforce this in code (e.g. an operator-configurable flag to disable guardrail resource surfacing entirely below `approved` status) is a reasonable follow-up, but should be a deliberate product decision made with the named safety/legal/partnerships owner, not a default this PR sets unilaterally.
...
- `backend/tests/guardrails/test_responses.py` — response composition: the localized intro plus deterministic resource lines, the safe-fallback-intro path (including on a simulated timeout), and that a failed generation call still raises when there are no resources to fall back on.
