...
class TestBaseCrawler:
    ...
    def test_get_file_path_from_url(self):
        ...
        expected_start = os.path.join(
            crawler.output_dir, "example.com", "page_param_value"
        )
    ...
    def test_save_url_mappings_exception(self):
        ...
        with (
            ...
            patch("tapio_crawler.crawler.crawler.logger.exception") as mock_log,
        ):
            crawler._save_url_mappings()
    ...
    async def test_crawl_url_success(self):
        ...
        mock_response.text = (
            "<html><body><h1>Test</h1><a href='/page2'>Link</a></body></html>"
        )
        ...
        with patch.object(
            crawler, "_save_html_content", return_value="/fake/path.html"
        ):
            results = []
    ...
    async def test_crawl_url_http_error(self):
        ...
        results = []
        with patch("tapio_crawler.crawler.crawler.logger.warning") as mock_log:
    ...
    async def test_crawl_url_semaphore_deadlock_prevention(self):
        ...
        with patch.object(
            type(crawler), "semaphore", MockSemaphore(original_semaphore)
        ):
            ...
            with patch.object(
                ...
                acquire_count = len(
                    [call for call in semaphore_calls if call.startswith("acquire")]
                )
                release_count = len(
                    [call for call in semaphore_calls if call.startswith("release")]
                )
                assert acquire_count == release_count, (
                    "Semaphore acquire/release mismatch"
                )
    ...
    async def test_crawl_multi_level_depth_without_deadlock(self):
        ...
        with patch.object(
            ...
            for expected_url in expected_urls:
                assert expected_url in requested_urls, (
                    f"Expected URL {expected_url} was not requested"
                )
    ...
    async def test_crawl_links_processed_outside_semaphore_context(self):
        ...
        with patch.object(
            ...
            parent_end_idx = next(
                ...
                if event == "http_request_end_https://example.com"
            )
    ...
    def test_get_file_path_from_url_path_traversal_protection(self):
        ...
        for malicious_url in malicious_urls:
            with pytest.raises(
                ValueError, match="Invalid URL results in path outside output directory"
            ):
        ...
        expected = os.path.join(
            crawler.output_dir, "example.com", "..%2F..%2F..%2Fetc%2Fpasswd.html"
        )
