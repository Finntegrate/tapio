...
class TestRelativeLinks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from tapio_crawler.config import settings as tapio_settings

        self._orig_content_dir = tapio_settings.DEFAULT_CONTENT_DIR
        tapio_settings.DEFAULT_CONTENT_DIR = self.temp_dir
        from tapio_crawler.config.settings import DEFAULT_DIRS

        self.input_dir = os.path.join(
            self.temp_dir, "test_site", DEFAULT_DIRS["CRAWLED_DIR"]
        )
        self.output_dir = os.path.join(
            self.temp_dir, "test_site", DEFAULT_DIRS["PARSED_DIR"]
        )
    ...
    def test_parse_file(self):
        ...
        self.assertIsNotNone(
            result, f"parse_file returned None for {self.test_html_path}"
        )
        # Accept both possible base URLs: with and without trailing path
        expected_base_urls = [
            f"https://{self.domain}/page-with-links",
            f"https://{self.domain}/",
        ]
        self.assertIn(
    ...
    def test_domain_specific_url_handling(self):
        ...
        domain_file_path = os.path.join(
            self.domain_dir, "subdir", "domain-specific.html"
        )
        ...
        self.assertIsNotNone(
            parser.current_base_url, "current_base_url is None after parsing"
        )
        self.assertTrue(
            parser.current_base_url
            and parser.current_base_url.startswith(f"https://{self.domain}")
        )
