...
class TestConfigManager:
    ...
    def test_init_custom_path(self, custom_site_config_yaml):
        ...
        with patch(
            "pathlib.Path.open", mock_open(read_data=custom_site_config_yaml)
        ) as mock_file:
    ...
    def test_invalid_config_structure(self):
        ...
        with (
            patch("pathlib.Path.open", mock_open(read_data="not_sites: {}")),
    ...
    def test_get_nonexistent_site_config(self, basic_config_yaml):
        ...
        with patch(
            ...
            config_manager = ConfigManager()
            with pytest.raises(
                ValueError, match="Site 'nonexistent' not found in configuration"
            ):
    ...
    def test_get_site_descriptions(self, site_with_descriptions_yaml):
        ...
        with patch(
            "pathlib.Path.open", mock_open(read_data=site_with_descriptions_yaml)
        ):
            config_manager = ConfigManager()
