...
class BaseCrawler:
    ...
    def __init__(
        ...
        self.output_dir = str(
            Path(DEFAULT_CONTENT_DIR) / self.site_name / DEFAULT_DIRS["CRAWLED_DIR"]
        )
    ...
    async def crawl(self) -> list[CrawlResult]:
        ...
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            ...
            tasks = [
                self._crawl_url(client, url, 0, results) for url in self.start_urls
            ]
    ...
    async def _crawl_url(
        ...
        async with self.semaphore:
            logger.info(
                "Processing %s at depth %s/%s", url, current_depth, self.max_depth
            )
            ...
            links_to_follow = await self._fetch_and_process(
                client, url, current_depth, results
            )
