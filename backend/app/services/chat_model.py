...
def build_chat_model(config: RAGConfig, llm_settings: LLMSettings, *, timeout: float | None = None) -> BaseChatModel:
    ...
    Args:
        ...
        llm_settings: Source of the ``api_base``/``api_key`` overrides used for Scaleway
            and other OpenAI-compatible endpoints (``TAPIO_LLM_API_BASE``/``TAPIO_LLM_API_KEY``,
            passed with ``TAPIO_LLM_PROVIDER=openai``).
